...
Transform decomposeTransformMatrix(const Eigen::Matrix4d& matrix) {
    Transform result;

    // 提取平移向量
    result.translation = matrix.block<3, 1>(0, 3); // 提取第四列的前三个元素

    // 提取旋转矩阵并转换为四元数
    Eigen::Matrix3d rotationMatrix = matrix.block<3, 3>(0, 0); // 提取左上角的3x3子矩阵
    result.quaternion = Eigen::Quaterniond(rotationMatrix);
...
bool MapUser::Relocalization(std::string image_id ,cv::Mat& image, Eigen::Matrix4d& pose){
  cv::Mat image_rect;
  ...
  if(_configs.ros_publisher_config.reloc){
    ...
    outFile.open("/my_workspace/SlamDemo/SlamDemo/Assets/StreamingAssets/CamTrans.txt");
    // 检查文件是否成功打开
    if (outFile.is_open()) {
